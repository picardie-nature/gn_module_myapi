...
class MyCustomQuery(CustomQuery) :
    ...
    def result_process(self, x): #TODO utilser les tempates jinja
        for i,e in enumerate(x) :
            image =  '<img src="https://taxhub.clicnat.fr/api/tmedias/thumbnail/{id_media}?h=200">'.format(id_media=e['id_media']) if e['id_media'] else ''
            description="""<p><a href='https://clicnat.fr/espece/{cd_nom}'><i>{lb_nom}</i></a> (<i>{fm}</i>) observé le {date_obs} par {observers}. </p> 
                <p>Commune de <a href="https://clicnat.fr/territoire/{commune_code}>{commune_name}</a> ({commune_code}).</p>
                <table>
                ...
                """.format(cd_nom=e['cd_nom'], lb_nom=e['lb_nom'], cl=e['classe'], ordre=e['ordre'], fm=e['famille'], date_obs=e['date_obs'].strftime('%d/%m/%Y'), observers=e['observers'], id_media=e['id_media'], commune_name=e['commune'], commune_code=e['commune_code'], pubDate=e['pub_date'].strftime('%d/%m/%Y'))
